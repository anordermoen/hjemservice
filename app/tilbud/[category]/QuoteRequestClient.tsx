...
export function QuoteRequestClient({
  ...
  const handleNextStep = (e: React.FormEvent) => {
    ...
    if (!isLoggedIn) {
      router.push(`/logg-inn?callbackUrl=/tilbud/${category.slug}`);
      return;
    }
    ...
    try {
      const response = await fetch("/api/quote-request", {
        ...
        body: JSON.stringify({
          ...
          selectedProviderIds: Array.from(selectedProviders),
        }),
      });
                  ...
                  <div className="space-y-2">
                    {filteredProviders.map((provider) => (
                      <ProviderSelectCard
                        key={provider.id}
                        provider={provider}
                        selected={selectedProviders.has(provider.id)}
          ...
          <CardContent>
            <form onSubmit={handleNextStep} className="space-y-6">
              ...
              <Button type="submit" className="w-full">
                {isLoggedIn ? "Neste: Velg leverandører" : "Logg inn for å fortsette"}
