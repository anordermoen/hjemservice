...
function BookingCard({
  ...
  const within24Hours = type === "upcoming" && isWithin24Hours(booking.scheduledAt);
    ...
    <div className="rounded-lg border p-4">
      ...
      <div className="grid gap-2 sm:grid-cols-2 mb-3">
        <div className="flex items-start gap-2 text-sm">
          ...
          <div>
            <p className="font-medium">{formatDate(new Date(booking.scheduledAt))}</p>
            <p className="text-muted-foreground">
              kl. {formatTime(new Date(booking.scheduledAt))} ({totalDuration} min)
            </p>
          </div>
        </div>
        {booking.address && (
      ...
      {booking.address && (booking.address.floor || booking.address.entryCode || booking.address.instructions) && (
...
export function ProviderBookingsClient({ bookings }: ProviderBookingsClientProps) {
  ...
  const upcoming = bookings.filter(
    (b) => b.status === "CONFIRMED" && new Date(b.scheduledAt) > now
  );
            ...
            <div className="py-4">
              <div className="rounded-lg bg-muted p-3 mb-4">
                ...
                <p className="text-sm text-muted-foreground">
                  {formatDate(new Date(selectedBooking.scheduledAt))} kl.{" "}
